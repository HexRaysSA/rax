...
impl Aarch64Lowerer {
    ...
    fn lower_neg(
        ...
        if !set_flags {
            if let VReg::Imm(value) = src {
                ...
                return self.emit_mov_imm(Self::dst_gpr(dst)?, value as i64, emit_width);
            }
        }
    ...
    fn lower_bfx(
        ...
        if let VReg::Imm(value) = src {
            ...
            return self.emit_mov_imm(Self::dst_gpr(dst)?, result as i64, op_width);
        }
        ...
        if signed && rem.is_none() {
            if let Some(imm) = Self::src_imm(src2) {
                if ((imm as u64) & width.mask()) == width.mask() {
                    return match width {
                        OpWidth::W32 | OpWidth::W64 => self.lower_neg(quot, src1, false, width),
                        other => Err(LowerError::UnsupportedOp {
...
mod tests {
    ...
    #[test]
    fn lowers_neg_w_imm_as_movz_movk() {
        let mut builder = FunctionBuilder::new(FunctionId(0), 0);
        builder.push_op(
            0,
            OpKind::Neg {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_mov_wide(0, 0b10, 0, 0xedcc, 0).to_le_bytes());
        expected.extend_from_slice(&enc_mov_wide(0, 0b11, 1, 0xffff, 0).to_le_bytes());
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
        assert_eq!(code, expected);
    }
    ...
    #[test]
    fn lowers_bfx_w_imm_sign_extend_as_movz_movk() {
        let mut builder = FunctionBuilder::new(FunctionId(0), 0);
        builder.push_op(
            0,
            OpKind::Bfx {
    ...
    #[test]
    fn lowers_not_w_imm_as_movz_movk() {
        let mut builder = FunctionBuilder::new(FunctionId(0), 0);
        builder.push_op(
            0,
            OpKind::Not {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_mov_wide(0, 0b10, 0, 0xedcb, 0).to_le_bytes());
        expected.extend_from_slice(&enc_mov_wide(0, 0b11, 1, 0xffff, 0).to_le_bytes());
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
        assert_eq!(code, expected);
    }
