...
impl Aarch64Lowerer {
    ...
    fn src_operand_is_zero(src: &SrcOperand) -> bool {
        match src {
        ...
        if !n && opc == 0b01 {
            if let VReg::Imm(imm) = src1 {
                ...
                if value == all_ones
                    && matches!(
                        src2,
                        SrcOperand::Reg(_) | SrcOperand::Shifted { .. } | SrcOperand::Extended { .. }
                    )
                {
                    let dst = Self::dst_gpr(dst)?;
    ...
    fn lower_bsf(
        ...
        if flags.updates_any() {
            self.lower_bit_scan_zero_flags(Self::dst_gpr(dst)?, src, width, mask_width)?;
        }
    ...
    fn lower_bsr(
        ...
        if flags.updates_any() {
            ...
            self.lower_bit_scan_zero_flags(Self::dst_gpr(dst)?, src, width, emit_width)?;
        }
    ...
    fn cls_imm(value: u64, width: OpWidth) -> Result<u64, LowerError> {
        match width {
            OpWidth::W32 => {
        ...
        if let Some(dst_hi) = dst_hi {
            ...
            if src1_zero || src2_zero {
                ...
                if !matches!(dst_lo, VReg::Virtual(_)) {
                    self.emit_mov_imm(Self::dst_gpr(dst_lo)?, 0, OpWidth::W64)?;
                }
                return self.emit_mov_imm(Self::dst_gpr(dst_hi)?, 0, OpWidth::W64);
            }
        }
        if let Some(dst_hi) = dst_hi {
            if let (VReg::Imm(lhs), Some(rhs)) = (src1, Self::src_imm(src2)) {
                ...
                if !matches!(dst_lo, VReg::Virtual(_)) {
                    self.emit_mov_imm_best(Self::dst_gpr(dst_lo)?, lo as i64, OpWidth::W64)?;
                }
                return self.emit_mov_imm_best(Self::dst_gpr(dst_hi)?, hi as i64, OpWidth::W64);
            }
        }
        if dst_hi.is_none()
    ...
    fn src_shift_count_eq(src: &SrcOperand, value: u32) -> bool {
        let Some(imm) = Self::src_imm(src) else {
            return false;
        };
    ...
    fn flagm_shl(op: &OpKind, dst: VReg, src: VReg, amount: u32) -> bool {
        matches!(
            op,
            OpKind::Shl {
                dst: op_dst,
    ...
    fn flagm_shr(op: &OpKind, dst: VReg, src: VReg, amount: u32) -> bool {
        matches!(
            op,
            OpKind::Shr {
                dst: op_dst,
    ...
    fn is_low_contiguous_mask(mask: u64, width: OpWidth) -> bool {
    ...
    fn lower_double_shift(
        ...
        if matches!(width, OpWidth::W8 | OpWidth::W16) {
            ...
            if amount == bits {
                if let VReg::Imm(value) = src {
                    let result = (value as u64) & width.mask();
                    return self.emit_mov_imm(dst_reg, result as i64, OpWidth::W32);
                }
                let src = Self::gpr(src)?;
...
mod tests {
    ...
    fn lowers_adds_x_zero_base_zero_imm_as_adds_zero_regs() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(
            &enc_addsub_shift_regs(1, 0, 1, 0, 0, 0, 31, 31).to_le_bytes(),
        );
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
        assert_eq!(code, expected);
    }
    ...
    fn lowers_subs_w_zero_base_masked_zero_imm_as_subs_zero_regs() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(
            &enc_addsub_shift_regs(0, 1, 1, 0, 0, 0, 31, 31).to_le_bytes(),
        );
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
        assert_eq!(code, expected);
    }
    ...
    fn lowers_divu_w8_imm_power_of_two_with_remainder_as_lsr_and() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_bitfield(0, 0b10, 3, 7).to_le_bytes());
        expected.extend_from_slice(&enc_logical_imm(0, 0b00, 0, 0, 2, 3, 1).to_le_bytes());
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
        assert_eq!(code, expected);
    }
    ...
    fn lowers_divu_w16_imm_power_of_two_remainder_before_aliasing_quotient() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_logical_imm(0, 0b00, 0, 0, 6, 3, 1).to_le_bytes());
    ...
    fn lowers_pdep_pext_low_masks_as_and_mask() {
        let cases = [
            (
                ...
                vec![enc_logical_imm(1, 0b00, 1, 0, 12, 0, 1), 0xd65f_03c0u32],
            ),
            (
                OpKind::Pext {
                    dst: x(0),
                ...
                vec![enc_logical_imm(0, 0b00, 0, 0, 4, 0, 1), 0xd65f_03c0u32],
            ),
            (
                OpKind::Pdep {
                    dst: x(0),
                ...
                vec![
                    enc_logical_imm(0, 0b00, 0, 0, 3, 0, 1),
                    enc_bitfield_regs(0, 0b10, 0, 7, 0, 0),
                    0xd65f_03c0u32,
                ],
            ),
    ...
    fn fuses_lifted_bfxil_imm_source_as_and_orr() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(
            &enc_logical_imm(1, 0b00, clear_n, clear_immr, clear_imms, 0, 1)
                .to_le_bytes(),
        );
        expected.extend_from_slice(
            &enc_logical_imm(1, 0b01, insert_n, insert_immr, insert_imms, 0, 0)
                .to_le_bytes(),
        );
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
        assert_eq!(code, expected);
    ...
    fn lowers_bsf_flag_setting_x_as_source_test_rbit_clz_ubfx() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_logical_reg_n(1, 0b11, 0, 31, 1, 1).to_le_bytes());
        expected.extend_from_slice(&enc_dp1_regs(1, 0b000000, 1, 0).to_le_bytes());
        expected.extend_from_slice(&enc_dp1_regs(1, 0b000100, 0, 0).to_le_bytes());
        expected.extend_from_slice(&enc_bitfield_regs(1, 0b10, 0, 5, 0, 0).to_le_bytes());
        expected.extend_from_slice(&0xd65f_03c0u32.to_le_bytes());
    ...
    fn lowers_bsf_flag_setting_w16_masks_source_test() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_logical_imm(0, 0b11, 0, 0, 15, 31, 1).to_le_bytes());
        expected.extend_from_slice(&enc_logical_imm(0, 0b01, 0, 16, 0, 0, 1).to_le_bytes());
        expected.extend_from_slice(&enc_dp1_regs(0, 0b000000, 0, 0).to_le_bytes());
        expected.extend_from_slice(&enc_dp1_regs(0, 0b000100, 0, 0).to_le_bytes());
    ...
    fn lowers_bsr_flag_setting_w8_masks_source_test() {
        ...
        let mut expected = Vec::new();
        expected.extend_from_slice(&enc_logical_imm(0, 0b11, 0, 0, 7, 31, 1).to_le_bytes());
        expected.extend_from_slice(&enc_bitfield_regs(0, 0b10, 0, 7, 1, 0).to_le_bytes());
