...
fn main() {
    ...
    let st = if thumb { rax::arm::ExecutionState::Thumb } else { rax::arm::ExecutionState::Aarch32 };
    ...
    for _ in 0..count {
        ...
        match d.decode(b) {
            Ok(i) => {
                let len = if thumb && (u16::from_le_bytes([b[0],b[1]])>>11) < 0x1D { 2 } else { 4 };
                let raw = if len==2 { format!("{:04x}", u16::from_le_bytes([b[0],b[1]])) } else { format!("{:08x}", u32::from_le_bytes([b[0],b[1],b[2],b[3]])) };
                println!("{:#010x}: {:<8} {:?} {:?}", va, raw, i.mnemonic, i.operands);
                va += len;
            }
            Err(_) => { println!("{:#010x}: ??", va); va += 2; }
        }
    }
}
