...
- process tracing (`ptrace`) between a process and its children or parent: `PTRACE_TRACEME`, `PTRACE_ATTACH`, `PTRACE_SEIZE`, `PTRACE_DETACH`, `PTRACE_KILL`, `PTRACE_CONT`, memory and user-area peeks and pokes (into read-only text too), the general registers (`PTRACE_GETREGS`, `NT_PRSTATUS`) and AArch64's `NT_ARM_SYSTEM_CALL`, siginfo, signal masks, options, and the event message; signal-delivery-stops, group stops, `execve`'s `SIGTRAP` and `PTRACE_EVENT_EXEC` stop, `wait` and `SIGCHLD` (`CLD_TRAPPED`) for the tracer, `TracerPid`, and a tracer's end detaching or killing its tracees; system-call stops (`PTRACE_SYSCALL`, `PTRACE_O_TRACESYSGOOD`) in each architecture's entry view, `PTRACE_SYSEMU` and `PTRACE_SYSEMU_SINGLESTEP` (x86-64, AArch64), `PTRACE_GET_SYSCALL_INFO` and `PTRACE_SET_SYSCALL_INFO` (a call changed, skipped, or given a result), and single-stepping (x86-64, AArch64) with each architecture's traps (unit-tested on every ABI; differential-tested by the `ptrace` and `ptracestops` fixtures, whose x86-64 and RV64 expectations are the AArch64 kernel's because Rosetta traces its translated processes itself and QEMU user mode lacks `ptrace`);
...
- process tracing reaches only a process's children and its parent (the links `fork` makes): any other process, which Linux with Yama's classic permissions would let a same-user tracer attach to, is refused with `EPERM`, and a thread of another process is reached only as that process's main thread; a parent that is not a `rax-user` process traces a `PTRACE_TRACEME` thread that never goes on from a stop; `PTRACE_INTERRUPT`, `PTRACE_LISTEN`, `PTRACE_PEEKSIGINFO`, x86-64's `PTRACE_SINGLEBLOCK` (branch stepping), the floating-point and other register sets, `PTRACE_O_TRACESECCOMP`, and the fork, clone, and exit events are not implemented yet (`EIO`, or `EINVAL` for a register set); debug registers read as zero and only zero can be written (`EIO`), so x86-64's `DR6` does not show a single step's `DR_STEP`, and segment selectors are checked but not kept; `PTRACE_O_SUSPEND_SECCOMP` is refused as on a kernel without checkpoint and restore (`EINVAL`);
