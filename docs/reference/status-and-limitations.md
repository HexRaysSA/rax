...
- 195 system calls across descriptors, paths, memory management, identity, limits, clocks, timers, signals, threads, futexes, processes, POSIX timers, and event, timer, and signal descriptors;
...
- `ITIMER_VIRTUAL`/`ITIMER_PROF` and CPU-time POSIX timers count the emulator's host CPU time, including emulation overhead, and other processes' CPU clocks are refused; the alarm clocks need root; `TFD_TIMER_CANCEL_ON_SET` never cancels (host clock changes are not observed); `/proc/<pid>/fdinfo` is absent;
...
- no socket creation or `epoll`; writable `MAP_SHARED` file mappings do not write back;
