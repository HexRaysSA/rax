...
- process tracing (`ptrace`) between a process and its children or parent, and of the processes its tracees fork: `PTRACE_TRACEME`, `PTRACE_ATTACH`, `PTRACE_SEIZE`, `PTRACE_DETACH`, `PTRACE_KILL`, `PTRACE_CONT`, memory and user-area peeks and pokes (into read-only text too), the general registers (`PTRACE_GETREGS`, `NT_PRSTATUS`), the floating-point registers (`NT_PRFPREG`, `PTRACE_GETFPREGS`, `PTRACE_SETFPREGS`), x86-64's XSAVE area (`NT_X86_XSTATE`) and `PTRACE_ARCH_PRCTL`, and AArch64's `NT_ARM_TLS` and `NT_ARM_SYSTEM_CALL`, siginfo, pending records (`PTRACE_PEEKSIGINFO`), signal masks, the rseq registration, options, and the event message; signal-delivery-stops, group stops, `execve`'s `SIGTRAP` and `PTRACE_EVENT_EXEC` stop, `wait` and `SIGCHLD` (`CLD_TRAPPED`, or `CLD_STOPPED` for a group stop's or `PTRACE_INTERRUPT`'s trap) for the tracer, a seized tracee's job control (`PTRACE_EVENT_STOP`, `PTRACE_INTERRUPT`, `PTRACE_LISTEN`, `SIGCONT`'s traps), the clone event for threads, the fork, vfork (then `PTRACE_EVENT_VFORK_DONE`), and clone events for processes, the new process traced from its first instruction (and `CLONE_PTRACE`, `CLONE_UNTRACED`), a root tracer's `PTRACE_O_SUSPEND_SECCOMP`, `PTRACE_SECCOMP_GET_FILTER`, and `PTRACE_SECCOMP_GET_METADATA` (as with checkpoint and restore), the exit event, the seccomp event (`SECCOMP_RET_TRACE`), a tracer reaping the traced threads that exit, `TracerPid`, and a tracer's end detaching or killing its tracees; system-call stops (`PTRACE_SYSCALL`, `PTRACE_O_TRACESYSGOOD`) in each architecture's entry view, `PTRACE_SYSEMU` and `PTRACE_SYSEMU_SINGLESTEP` (x86-64, AArch64), `PTRACE_GET_SYSCALL_INFO` and `PTRACE_SET_SYSCALL_INFO` (a call changed, skipped, or given a result), single-stepping (x86-64, AArch64) with each architecture's traps, block-stepping (x86-64: a trap only after a branch), and x86-64's I/O permission bitmap and shadow-stack sets (no contents: `ENXIO`, `ENODEV`) (unit-tested on every ABI; differential-tested by the `ptrace`, `ptracestops`, `ptraceregs`, `ptracejobs`, `ptraceevents`, `ptracefork`, `ptraceseccomp`, and `ptraceblock` fixtures, whose RV64 expectations (and, but for `ptracejobs` and `ptracefork`, x86-64 ones) are the AArch64 kernel's because Rosetta traces its translated processes itself (and refuses seccomp) and QEMU user mode lacks `ptrace`);
...
- i386 (ELF32) programs run as compatibility tasks with an early subset of the 32-bit table (process exit, `read`, `write`, `readv`, `writev`, `close`, `brk`, `mmap`, `mmap2`, `munmap`, `mprotect`, `madvise`, `set_thread_area`, `get_thread_area`, `set_tid_address`, `uname`, `dup*`, `pipe*`, `memfd_create`, `getrandom`, the kill calls, the 32-bit ID getters, and the terminal `ioctl`s); every other call returns `ENOSYS`, so signal handlers, threads, file-system calls, sockets, and `ptrace` of i386 programs are not available yet; `SYSENTER` raises `SIGILL` (there is no 32-bit vDSO), and compatibility-mode code does not use the JIT;
...
- process tracing reaches only a process's children and its parent (the links `fork` makes) and the processes its tracees fork (links they pass it): any other process, which Linux with Yama's classic permissions would let a same-user tracer attach to, is refused with `EPERM`, and a thread of another process is reached only as that process's main thread; a parent that is not a `rax-user` process traces a `PTRACE_TRACEME` thread that never goes on from a stop; x86-64's `PTRACE_GET_THREAD_AREA` and `PTRACE_SET_THREAD_AREA`, the register sets beyond the general, floating-point, XSAVE, TLS, and system-call ones (vector, SVE, hardware breakpoints), and the vfork events of a `CLONE_VFORK` thread are not implemented yet (`EIO`, or `EINVAL` for a register set); the emulated x86-64 CPU does not track the init state of XSAVE components, so `NT_X86_XSTATE` marks every enabled component in use; a traced process's group stop stops its traced threads only (its untraced threads go on), its real parent is not told of it or of its end, the real parent of a process traced by another learns of its exit (`SIGCHLD`, `wait`) from the host as it happens rather than once the tracer has reaped it, and `SIGSTOP` from a process other than a child or the parent, or to a process group, still stops the host process of a traced receiver; debug registers read as zero and only zero can be written (`EIO`), so x86-64's `DR6` does not show a single step's `DR_STEP`, and segment selectors are checked but not kept; only root holds `CAP_SYS_ADMIN` for `PTRACE_O_SUSPEND_SECCOMP` and the seccomp filter queries;
