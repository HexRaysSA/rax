...
- 219 system calls across descriptors, paths, memory management, identity, limits, clocks, timers, signals, threads, futexes, processes, POSIX timers, event, timer, and signal descriptors, `epoll`, and sockets;
...
- `ITIMER_VIRTUAL`/`ITIMER_PROF` and CPU-time POSIX timers count the emulator's host CPU time, including emulation overhead, and other processes' CPU clocks are refused; the alarm clocks need root; `TFD_TIMER_CANCEL_ON_SET` never cancels (host clock changes are not observed); `/proc/<pid>/fdinfo` is absent; an `epoll` instance is copied by `fork`, readiness from outside the emulator reaches edge-triggered items once per growth and in insertion order, and the watch and wake-up-path limits are not enforced;
- a `CLONE_VM` child process is a copy (`vfork` parent sleeps correctly but does not see the child's stores); processes sharing tables with their parent, `CLONE_PARENT`, pidfds, `CLONE_INTO_CGROUP`, namespaces, and the requeue-PI futex operations are unsupported; signals to other processes are host signals (no `sigqueue` values, real-time signals only to the sender, `kill(-1)` limited to children, and on macOS hosts a signal sent to a child the host is still creating may lose its sender) and `PR_SET_PDEATHSIG` is never delivered; a thread must share the descriptor table and file-system context; all threads of a process share one emulated CPU;
...
- a private file mapping copies each page at its first touch (later file changes do not reach it); a shared mapping of a block device is a copy; another process truncating a file mapped shared here, past a page touched here, faults `rax-user` instead of raising the guest's `SIGBUS`;
