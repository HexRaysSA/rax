...
- 177 system calls across descriptors, paths, memory management, identity, limits, clocks, timers, signals, threads, and futexes;
...
- new processes (`fork`, `vfork`, non-thread `clone`) and `execve` are unsupported; a thread must share the descriptor table and file-system context; `CLONE_PIDFD`, `CLONE_INTO_CGROUP`, namespaces, and the requeue-PI futex operations are unsupported; all threads share one emulated CPU;
