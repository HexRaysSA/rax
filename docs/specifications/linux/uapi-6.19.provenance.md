...
- License: the headers carry `GPL-2.0 WITH Linux-syscall-note` (59 files),
...
- open, `fcntl`, `mmap`, `clone`, `futex`, `prctl`, and signal constant
...
- structure layouts marshalled across the ABI (`struct stat`, `struct statx`,
  `struct timespec`, `struct rlimit`, `struct utsname`, signal frames);
