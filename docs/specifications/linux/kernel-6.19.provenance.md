...
- Source URL: https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git
  ...
  `include/uapi/linux/{aio_abi,fs}.h`) came from kernel.org's
...
- Retrieved: 24 September 2026 (`drivers/perf/riscv_pmu_sbi.c`,
  ...
  memory locking and sealing files, the restartable sequences files, and
  the asynchronous I/O files: 25 September 2026)
- Integrity: `kernel-6.19.sha256` lists the SHA-256 of every imported file,
  relative to `kernel-6.19/`.
- License: the files carry an SPDX identifier: `GPL-2.0` (104 files),
