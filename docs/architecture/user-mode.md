...
- **System V IPC** (`ipc`, `syscall::ipc`). The objects of every
  ...
  (`EINTR`). Undo adjustments are applied at exit, or, for a killed
  process, when the table is next read after it is gone. A message queue
...
- **Scheduling attributes** (`priority`, `syscall::priority`). Each
  ...
  slice, deadline parameters, timer slack, and I/O priority. The calls
...
- **System calls.** 305 calls across descriptors and I/O, paths and
...
| Scheduling attributes | `src/user/linux/tests/priority.rs` (8 tests, on every ABI) and the unit tests of `src/user/linux/priority.rs`: defaults, the priority ranges, `sched_setscheduler`'s checks in order and the permission rules, `sched_setattr`'s sizes, flags, slices, `SCHED_IDLE`, and kept policy or parameters, `sched_getattr`, deadline admission and `EAGAIN` at `clone`, time slices, nice values (a thread, the group, the user, a partial failure), inheritance, `reset_on_fork`, timer slack, `/proc/<pid>/task/<tid>/stat`, and I/O priorities |
...
| System V semaphores | `src/user/linux/tests/sysvsem.rs` (3 tests on every ABI): values and `semctl`'s commands with each ABI's `semid64_ds`, operation lists all or none and in order, `semtimedop`'s checks in order and its timeout, waits counted by `GETNCNT` and `GETZCNT` and ended by a value, a removal, or a signal, `SEM_UNDO` at exit; unit tests of `perform_atomic_semop`'s rules and `exit_sem` in `src/user/linux/ipc/sem.rs` |
