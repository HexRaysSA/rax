...
- **Process memory and kernel objects** (`syscall::task`,
  ...
  local fault, exactly, with the bytes moved so far. Vectors are imported
  as `__import_iovec` imports them (the count an `unsigned int`, a single
  vector capped at `MAX_RW_COUNT` before `access_ok`, several checked at
  full length). `kcmp` compares the objects modelled per task: the
...
- **System calls.** 309 calls across descriptors and I/O, paths and
  metadata (with device and FIFO nodes, file times, and extended
  attributes), memory management (with `memfd_create` and access through
  a task's ID), identity (with
