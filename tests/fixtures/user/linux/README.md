...
- The expected results were recorded with `record-expected.sh` on the
  ...
  kernel's AArch64 status-flag encoding in `fdinfo` (both, `fdinfo`), the
  native AArch64 result is used for architecture-independent kernel code. Rosetta has once, in
