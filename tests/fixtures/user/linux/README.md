...
- The expected results were recorded with `record-expected.sh` on the
  ...
  faults on `utime` with an unmapped buffer (Rosetta, `nodes`), the native
  AArch64 result is used for architecture-independent kernel code. Rosetta has once, in
