...
| `misc` | `getgroups` (the count, a small or bad buffer, a negative size) and the `Groups:` line of `/proc/self/status`; `setgroups` for root (sorting, the invalid gid, `NGROUPS_MAX`, a bad buffer, clearing) and refused without `CAP_SETGID`; `readahead` on a file, a `/proc` file, and refused files (write-only, `O_PATH`, closed, a directory, a pipe, a socket); `sync_file_range`'s flag, range, and file checks |
...
- The expected results were recorded with `record-expected.sh` on the
  ...
  `*xattrat` calls (QEMU, `xattr`), the native AArch64 result is used for
