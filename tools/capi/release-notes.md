...
Every SDK is accompanied by a `-debug` archive built from the same compilation, holding the debug information split out of the shipped libraries: `librax.so.debug` plus the unstripped `librax.a` on Linux, `librax.dylib.dSYM` plus the unstripped `librax.a` on macOS, and `rax.pdb` on Windows. The SDK archive alone is enough to build and run; download the matching `-debug` archive when you need symbols in a debugger or crash report. `build-info.json` lists the sidecars under `debug_info` and the level under `debug`. That level is Cargo's `limited`: function names, source files and line numbers, which covers symbolicated backtraces, profilers and crash reports, but not local variable inspection.
