...
def main():
    ...
    arm = assemble(all_lines, "armv7a", "+dsp")
    thumb = assemble(all_lines, "thumbv7a", "+dsp")

    entries = []  # (label, insn, mode)
    dedup = set()
    for asm in all_lines:
        if asm in arm:
            v = to_insn(arm[asm], 0)
            key = (v, 0)
            if key not in dedup:
        ...
        if asm in thumb:
            r = to_insn(thumb[asm], 99)  # mode decided by length
            insn, mode = r
            key = (insn, mode)
            if key not in dedup:
                dedup.add(key)
    ...
    with open(out, "w") as f:
        ...
        f.write("// Exhaustive A32 + T16/T32 integer register-data-processing\n")
        f.write("// encoding table for the AArch32 differential sweep.\n")
        f.write("// Tuple: (label, instruction-word, mode) where mode 0=ARM 1=T16 2=T32.\n\n")
        f.write("pub static A32_SWEEP: &[(&str, u32, u8)] = &[\n")
        for label, insn, mode in entries:
            esc = label.replace("\\", "\\\\").replace('"', '\\"')
            f.write(f'    ("{esc}", {insn:#010x}, {mode}),\n')
        f.write("];\n")
    print(f"wrote {len(entries)} entries to {out}")
    na = sum(1 for e in entries if e[2] == 0)
    n1 = sum(1 for e in entries if e[2] == 1)
    n2 = sum(1 for e in entries if e[2] == 2)
    print(f"  A32={na}  T16={n1}  T32={n2}")
